...
public class PlayerGeneralMultiplayer : MonoBehaviour
{

    [SerializeField] private Vector3[] respawnPositions;
    ...
    // called on awake to populate scene with respawn locations for use in respawn method
    private void InitializeRespawnLocations()
    {
        
        
        int positionCount = respawnPositions.Length;
        respawnLocations = new Transform[positionCount];

        for (int i = 0; i < positionCount; i++)
        {
            GameObject respawnLocation = new GameObject($"RespawnLocation {i}");
            respawnLocation.transform.position = respawnPositions[i];
            respawnLocation.transform.SetParent(transform.parent);
            respawnLocations[i] = respawnLocation.transform;
        }
    }
    ...
    {
        // Choose a random respawn location from the array
        int randomIndex = Random.Range(0, respawnLocations.Length);
        Transform respawnLocation = respawnLocations[randomIndex];

        
        
        
        
        // Set the player's position to the chosen respawn location
        ...
        transform.rotation = respawnLocation.rotation;
         ...
         // undoes what happens when player dies @TakeDamage()
        isAlive = true;
        isAlive = true;
        statsReset();
